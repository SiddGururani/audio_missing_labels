...
if __name__ == "__main__":
    ...
    config = {
        'logdir': '../logs/LE',
        'exp_name': 'labelprune_droplabels',
        'mode': 0,
        'coarse': 0,
        'data_path': '../data',
        'hparams': {
    ...
    #     run(config)
    # config.pop('seed')
    # json.dump(config, open('../configs/labelprune_{}.json'.format(config['dataset']), 'w'))
    ...
    for drop_rate in label_drop_rates:
        for seed in seeds:
            ...
            run(config)
        config.pop('seed')
        json.dump(config, open('../configs/labelprune_droplabels_{}.json'.format(config['dataset']), 'w'))
